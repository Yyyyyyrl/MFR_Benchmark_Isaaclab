#!/usr/bin/env python3
"""RMA 2-Stage Training Script for Allegro continuous screwdriver turning (Isaac Lab).
...
Usage:
    ...
    python scripts/train_rma_allegro.py \
        --task Isaac-Allegro-Screwdriver-Continuous-Turning-Direct-v0 \
        --output outputs/allegro-continuous --continuous_curriculum \
        --num_envs 16384 --stage 1 --headless

    # Stage 2: Student adaptation module
    python scripts/train_rma_allegro.py \
        --task Isaac-Allegro-Screwdriver-Continuous-Turning-Direct-v0 \
        --stage 2 --num_envs 8192 --headless \
        --checkpoint outputs/allegro-continuous/<run>/best.pth
...
# ---- CLI arguments ----
parser = argparse.ArgumentParser(description="RMA 2-Stage Training for Allegro Screwdriver Continuous Turning")

# Environment args
parser.add_argument("--task", type=str,
                    default="Isaac-Allegro-Screwdriver-Continuous-Turning-Direct-v0",
...
# Default horizon_length=32: at decimation=6 (600-step episodes) this gives
# 32/600 ≈ 5 % of an episode per rollout, comparable to HORA's setup.
...
def main():
    ...
    ppo_config = {
        ...
        "mini_epochs": 5,
        "e_clip": 0.2,
        # gamma=0.999 at decimation=6 (0.1 s/step): effective horizon
        # 1/(1-0.999)=1000 steps = 100 s, well-matched to the 60 s episode.
        # Move this together with env decimation.
        "gamma": 0.999,
        "tau": 0.95,
        "entropy_coef": 0.0,
        "critic_coef": 4.0,
