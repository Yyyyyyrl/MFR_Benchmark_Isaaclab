...
parser.add_argument("--prop_hist_len", type=int, default=12)
parser.add_argument("--privileged_obs_dim", type=int, default=14)
parser.add_argument("--history_obs_dim", type=int, default=27)
...
def main():
    ...
    # Faster rendering: render every RL step
    ...
    env = env.unwrapped
    actions_low = torch.tensor(env.single_action_space.low, dtype=torch.float32, device="cuda:0")
    actions_high = torch.tensor(env.single_action_space.high, dtype=torch.float32, device="cuda:0")
    ...
    net_config = {
        ...
        "actions_num": env.single_action_space.shape[0],
        "input_shape": (obs_dim,),
        "priv_info": True,
    ...
    if use_adapt and "sa_mean_std" in checkpoint:
        sa_mean_std = RunningMeanStd((args_cli.prop_hist_len, args_cli.history_obs_dim)).to("cuda:0")
        sa_mean_std.eval()
    ...
    while simulation_app.is_running():
        ...
        mu = torch.max(torch.min(mu, actions_high), actions_low)
        ...
        if step % 60 == 0:
            ...
            if euler is not None:
                turns = extras.get("eval_continuous_turns")
                if turns is not None:
                    turn_velocity = extras.get("eval_turn_velocity")
                    turn_velocity_text = ""
                    if turn_velocity is not None:
                        turn_velocity_text = f"  turn_vel={turn_velocity[0]:6.2f}"
                    print(f"  step={step:4d}  reward={reward_mean:8.2f}  "
                          f"turns={turns[0]:6.2f}{turn_velocity_text}  "
                          f"euler=[{euler[0,0]:6.2f}, {euler[0,1]:6.2f}, {euler[0,2]:6.2f}]", end="\r")
                else:
                    print(f"  step={step:4d}  reward={reward_mean:8.2f}  "
                          f"euler=[{euler[0,0]:6.2f}, {euler[0,1]:6.2f}, {euler[0,2]:6.2f}]", end="\r")
